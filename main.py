...
st.set_page_config(page_icon="EquityGPT.png",layout="wide",page_title="EquityGPT")
st.markdown("<h1 style='text-align: center; font-weight:bold; font-family:comic sans ms; padding-top: 0rem;'> \
            EquityGPT by Paul Cheng</h1> ", unsafe_allow_html=True)

st.markdown("<h2 style='text-align: center;padding-top: 0rem;'>Creating insightful equity analysis using Natural Language \
            with ChatGPT</h2>", unsafe_allow_html=True)

st.sidebar.markdown("<h4  style='text-align: left;font-size:small;color:grey;padding-top: 0rem;padding-bottom: .2rem;'>EquityGPT</h4>", unsafe_allow_html=True)
st.sidebar.markdown('Created by <a style="text-align: center;padding-top: 0rem;" href="https://www.linkedin.com/in/paul-cheng/">Paul Cheng</a>', unsafe_allow_html=True)
...
if "datasets" not in st.session_state:
    datasets = {}
    # Preload datasets
    datasets["USA"] = pd.read_csv("USA.csv")
    datasets["UK"] =pd.read_csv("UK.csv")
    st.session_state["datasets"] = datasets
else:
    # use the list already loaded
    datasets = st.session_state["datasets"]

my_key = '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' #st.text_input(label = ":key: OpenAI Key:", help="Please ensure you have an OpenAI API account with credit. ChatGPT Plus subscription does not include API access.",type="password")
